/*
  changes:
 - add functionallity for background image
 - instead of hex color, you can also add image path
 */
        ...
        { // General:
          ...
          { // BackgroundColor: *optional
            ...
            if (keyReturnStr != null) {
              ...
              if (!checkHex(keyReturnStr)) {
                // 1.3.12 Change (Comment out):
                //sendErrorCode("\"\\config.ini\"\n[clock_general_" + (x + 1) + "] background = \"" + keyReturnStr + "\"\nINVALID HEX");
                //return;
                clockWindowManager[x].backGroundImage = loadImage(keyReturnStr);
              } else {
          ...
          { // Shadow_x: *optional
            ...
            if (keyReturnStr != null) {
              ...
              clockWindowManager[x].shadowOffset_x = int(keyReturnStr);
            }
          }
          { // Shadow_y: *optional
            ...
            if (keyReturnStr != null) {
              ...
              clockWindowManager[x].shadowOffset_y = int(keyReturnStr);
            }
          }
        }
...
void draw() {
  if (!configError) {
    ...
    for (int x = 0; x < numberOfClocks; x++)
    {
      ...
      if (cache.fileBytes.length > x) {
        cache.fileBytes[x] = (byte)clockWindowManager[x].screenNum();
      } else {
        byte[] oldByte = new byte[cache.fileBytes.length];
        for (int y = 0; y < cache.fileBytes.length; y++) {
          oldByte[y] = cache.fileBytes[y];
  ...
  void receive(byte[] data) {
    if (_udpListen) {
      ...
      if (msg.indexOf("visible") == 0) {
        ...
        if (msg.equals("cycle")) {
          int screenNumPrev = _screenNum;
          screenNumPrev++;
          if (screenNumPrev > GraphicsDevices.getScreenDevices().length) {
            screenNumPrev = 1;
          }
          setScreen(screenNumPrev);
...
class ClockWindow extends PApplet {
  ...
  void setup() {
    ...
    if (_clockWinManagerObj.backGroundImage != null) {
      float rw = float(_clockWinManagerObj.backGroundImage.width) / float(width);
      float rh = float(_clockWinManagerObj.backGroundImage.height) / float(height);
      imageMode(CENTER);

      if (rw < rh)
        _clockWinManagerObj.backGroundImage.resize(int(_clockWinManagerObj.backGroundImage.width / rw), int(_clockWinManagerObj.backGroundImage.height / rw));
      else
      _clockWinManagerObj.backGroundImage.resize(int(_clockWinManagerObj.backGroundImage.width / rh), int(_clockWinManagerObj.backGroundImage.height / rh));
    }
  ...
  void draw() {
    ...
    if (_vis)
    {
      ...
      if (_clockWinManagerObj.backGroundImage != null) {
        image(_clockWinManagerObj.backGroundImage, width / 2, height / 2);
      }
